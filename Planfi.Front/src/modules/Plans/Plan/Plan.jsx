...
const Plan = (props) => {
  ...
  const { loading, error, data, refetch: _refetch } = useQuery(PLANSEXERCISES)
  const refreshPlanExerciseData = useCallback(() => {
  ...
  if (data) {
    results = !searchTerm
      ...
      : data.serializedExercisesInstances.filter((exercise) =>
